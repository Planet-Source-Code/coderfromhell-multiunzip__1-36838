...
Public Function UZDLLPass(ByRef p As UNZIPCBCh, _
  ...
  If Test = True Then
    ...
    PasswordNeeded = True
  Else
    If UsePassword And MyPassword <> "" Then
      szpassword = MyPassword
    Else
      szpassword = InputBox("Please Enter The Password For File:" & vbCrLf & CurZipFile, "Password Required.")
    End If
  End If
